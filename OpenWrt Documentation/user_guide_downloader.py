...
urls = [url + CMD for url in urls if "docs/guide-user" in url] # extract only the user guide urls

try:
    os.mkdir(f"{os.getcwd()}/OpenWrtWikiPages")
except: 
    pass
...
for url in tqdm(urls):
    ...
    try:
        with open(f"OpenWrtWikiPages/{filename}.txt", "xb") as file:
